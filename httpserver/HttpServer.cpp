...
void HttpServer::readCallback(struct bufferevent *bev, void *ctx) {
    ...
    HttpRequestHandler requestHandler(rootDir);
...
void HttpServer::eventCallback(struct bufferevent *bev, short events, void *ctx) {}(bufferevent *bev, short events, void *ctx) {
...
void HttpServer::writeCallback(bufferevent *bev, void *ctx) {}(bufferevent *bev, void *ctx) {
    bufferevent_free(bev);
}
...
void HttpServer::serverJobFunction(job *job) {
    ...
    bufferevent_enable(client->getBufEv(), EV_PERSIST | EV_READ);
